...
/// passing to Cedar's WASM `isAuthorized()` or equivalent evaluation engine.
///
/// All entity UIDs are formatted as Cedar EntityUID strings
/// (e.g., `MyServer::User::"alice"`). Entities are serialized as a JSON
/// string (array of entity objects).
#[derive(Clone, Debug)]
pub struct AuthorizationComponents {
    /// The principal EntityUID as a Cedar string.
    pub principal: String,
    /// The action EntityUID as a Cedar string.
    pub action: String,
    /// The resource EntityUID as a Cedar string.
    pub resource: String,
    /// The entities serialized as a JSON array string.
    pub entities_json: String,
}

#[derive(Clone, Debug)]
/// A `RequestGenerator` is a structure produced by a `SchemaGenerator` which allows
/// a user to generate Cedar Requests that are valid for the Schema generated by the
/// `SchemaGenerator`.
...
impl RequestGenerator {
    ...
    pub fn generate_request(
        ...
        type_defs.extend(tool.inputs().type_definitions(), Some(input_ns.clone()));

        let (inputs, new_entities) =
            self.values_to_cedar(input.get_args(), &type_defs, Some(&input_ns))?;
    ...
    /// representations of Cedar authorization requests rather than the Rust
    /// types directly — for example, WASM bindings or REST API bridges.
    ///
    /// Returns the principal, action, and resource as Cedar EntityUID strings,
    /// and the entities as a JSON array. The caller is responsible for
    /// constructing context from the MCP tool input.
    pub fn generate_request_components(
        &self,
        input: &Input,
        principal: EntityUID,
        resource: EntityUID,
        ...
        let (request, result_entities) =
            self.generate_request(principal, resource, context, entities, input, output)?;

        // Serialize principal, action, resource via Display
        let principal_str = request
            .principal()
            .uid()
            .map(|u| u.to_string())
            .unwrap_or_default();
